...
export default function EditClientPage() {
  ...
  const [form, setForm] = useState({
    ...
    gstin: '', creditLimit: '', clientType: 'walk-in', status: 'active',
    preferredContact: 'phone', industry: '', notes: '',
  })
    ...
    async function fetchClient() {
      ...
      if (data.client) {
        ...
        setForm({
          ...
          clientType: c.clientType || 'walk-in',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setTimeout(() => router.push(`/dashboard/clients-vendors/clients/${id}`), 1500)
    } catch {
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '20px', fontWeight: '600', color: '#0f172a' }}>Edit client</h1>
      ...
      {/* Status change — prominent at top */}
      <div style={{ background: '#fff', border: '0.5px solid #e2e8f0', borderRadius: '10px', padding: '16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '20px' }}>
        <div style={{ fontSize: '13px', fontWeight: '600', color: '#0f172a' }}>Client status</div>
        <div style={{ display: 'flex', gap: '10px' }}>
          {[
            { value: 'active', label: '✓ Active', bg: form.status === 'active' ? '#eaf3de' : '#f8fafc', color: form.status === 'active' ? '#3b6d11' : '#64748b', border: form.status === 'active' ? '#3b6d11' : '#e2e8f0' },
            { value: 'hold', label: '⏸ On Hold', bg: form.status === 'hold' ? '#faeeda' : '#f8fafc', color: form.status === 'hold' ? '#854f0b' : '#64748b', border: form.status === 'hold' ? '#854f0b' : '#e2e8f0' },
            { value: 'blacklist', label: '✗ Blacklist', bg: form.status === 'blacklist' ? '#fcebeb' : '#f8fafc', color: form.status === 'blacklist' ? '#a32d2d' : '#64748b', border: form.status === 'blacklist' ? '#a32d2d' : '#e2e8f0' },
          ].map(opt => (
            <button
              key={opt.value}
              type="button"
              onClick={() => setForm(prev => ({ ...prev, status: opt.value }))}
              style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', background: opt.bg, color: opt.color, border: `1px solid ${opt.border}` }}
            >
              {opt.label}
            </button>
          ))}
        ...
        <div style={{ fontSize: '12px', color: '#64748b', marginLeft: 'auto' }}>
          {form.status === 'hold' && '⚠ Client on hold — no new jobs should be created'}
          {form.status === 'blacklist' && '🚫 Client blacklisted — block all transactions'}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ background: '#fff', border: '0.5px solid #e2e8f0', borderRadius: '10px', padding: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            ...
            <div><label style={labelStyle}>Credit limit (₹)</label><input name="creditLimit" type="number" value={form.creditLimit} onChange={handleChange} min="0" style={inputStyle} /></div>
          </div>

          <div style={sectionTitle}>Billing</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            <div><label style={labelStyle}>GSTIN</label><input name="gstin" value={form.gstin} onChange={handleChange} style={inputStyle} /></div>
          </div>

          <div style={sectionTitle}>Internal notes</div>
          <div style={{ marginBottom: '14px' }}>
            <textarea name="notes" value={form.notes} onChange={handleChange} rows={3} placeholder="Internal notes about this client..." style={{ ...inputStyle, height: 'auto', padding: '10px 12px', resize: 'vertical' as const }} />
          </div>
